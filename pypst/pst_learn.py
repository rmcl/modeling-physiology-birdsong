...
def pst_learn(
    ...
    while sequence_queue_sbar:
        ...
        f_vec_sum = f_vec_values.sum() + eps
        f_suf_sum = f_suf_values.sum() + eps

        p_sigma_s_values = f_vec_values / f_vec_sum
        p_sigma_suf_values = f_suf_values / f_suf_sum
        ...
        for i in range(p_sigma_s_indices.size(1)):  # Iterate over columns of the indices tensor
            ...
            #print('SIGMA SUF SHAPE', p_sigma_suf.shape)
            #print('SIGM_S_VAL SHAPE', p_sigm_s_val.shape)
            #print('INDEX ', index, 'SHAPE', index.shape)

            cur_index_ratio = (p_sigm_s_val + eps) / (p_sigma_suf_val + eps)

            total += cur_index_ratio
        ...
        if len(cur_sequence_indexes) < L:
            ...
            for i in range(f_vec_prime_indexes.size(1)):
                ...
                if p_sigmaprime_s >= p_min:
                    #new_sequence = [alphabet[f_vec_prime_index]] + cur_sequence
                    new_sequence = []

                    #print('NEW SEQ FPRIME', f_vec_prime_index)
                    for idx in f_vec_prime_index:
                        new_sequence.append(alphabet[idx])
...
def retrieve_f_sigma_sparse(f_mat, s) -> Tuple[torch.Tensor, torch.Tensor, Tuple[int]]:
    ...
    # Return the filtered sparse indices and values
    return filtered_indices, filtered_values, shape
